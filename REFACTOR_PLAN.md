...
36. **偶发静默退出(未结案,已加诊断)**。回归中途服务器两次**静默退出**
    ...
    已做:①`ServerApp::run` 把 `app.run()` 包 try/catch,**主循环自己返回或
    抛异常都会写日志**(`run_returned` / `run_exception`),下次复现就能定位;
    ②`/MAP` 本来就开(链接选项),崩溃偏移可符号化。**建议下一步专项**:
    在 sim 线程各段加 try/catch + 帧号,并用 WER LocalDumps 抓 dump
